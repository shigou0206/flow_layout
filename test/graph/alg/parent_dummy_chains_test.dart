...
void main() {
  group('parentDummyChains', () {
    ...
    test('handles nested subgraphs', () {
      ...
      print(
          'sg1 rank range: ${g.node('sg1')?['minRank']} - ${g.node('sg1')?['maxRank']}');
      print(
          'sg2 rank range: ${g.node('sg2')?['minRank']} - ${g.node('sg2')?['maxRank']}');
      print(
          'sg3 rank range: ${g.node('sg3')?['minRank']} - ${g.node('sg3')?['maxRank']}');
      print(
          'sg4 rank range: ${g.node('sg4')?['minRank']} - ${g.node('sg4')?['maxRank']}');
      ...
      expect(
          g.parent('d5'),
          equals(
              'sg3')); // Actual behavior shows d5 is assigned to sg3, not sg4
    });
