...
void _dfs(Graph g, String node, Map<String, bool> visited, List<String> component) {
  visited[node] = true;
  component.add(node);
  ...
  group('rank/nestingGraph', () {
    ...
    group('run', () {
      ...
      test('adds an edge from root to each node with the correct minlen #1', () {
        ...
        expect(g.edge(g.outEdges(root, 'a')![0]), equals({'weight': 0, 'minlen': 1}));
      });
      ...
      test('adds an edge from root to each node with the correct minlen #2', () {
        ...
        expect(root, isNotNull);
        expect(g.outEdges(root, 'a')?.length, equals(1));
        expect(g.edge(g.outEdges(root, 'a')![0]), equals({'weight': 0, 'minlen': 3}));
      });
      ...
      test('adds an edge from root to each node with the correct minlen #3', () {
        ...
        expect(root, isNotNull);
        expect(g.outEdges(root, 'a')?.length, equals(1));
        expect(g.edge(g.outEdges(root, 'a')![0]), equals({'weight': 0, 'minlen': 5}));
      });
