...
void main() {
  ...
  group('greedy-specific functionality', () {
    test('prefers to break cycles at low-weight edges', () {
      ...
      print(
          'Before run: ${g.edges().map((e) => '${e['v']}->${e['w']}').join(', ')}');
      ...
      print(
          'After run: ${g.edges().map((e) => '${e['v']}->${e['w']}').join(', ')}');
      ...
      // 低权重的边应该被反转
      expect(g.hasEdge('c', 'a'), isFalse,
          reason: 'The lowest weight edge c->a should be reversed');
      expect(g.hasEdge('a', 'c'), isTrue,
          reason: 'The reversed direction a->c should exist');
