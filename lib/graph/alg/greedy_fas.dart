...
List<Map<String, dynamic>> greedyFAS(
  ...
  final results = _doGreedyFAS(
    state['graph'] as Graph, 
    state['buckets'] as List<ListQueue<dynamic>>, 
    state['zeroIdx'] as int
  );
...
/// 执行贪心FAS算法
List<Map<String, dynamic>> _doGreedyFAS(Graph g, List<ListQueue<dynamic>> buckets, int zeroIdx) {
...
/// 从图中移除节点并更新桶
List<Map<String, dynamic>>? _removeNode(Graph g, List<ListQueue<dynamic>> buckets, int zeroIdx, dynamic entry, [bool collectPredecessors = false]) {
...
/// 构建算法需要的初始状态
Map<String, dynamic> _buildState(Graph g, dynamic Function(Map<String, dynamic>) weightFn) {
...
/// 将节点分配到相应的桶
void _assignBucket(List<ListQueue<dynamic>> buckets, int zeroIdx, dynamic entry) {
  if (entry['out'] == 0) {
